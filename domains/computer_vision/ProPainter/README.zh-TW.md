...
### 資料流對照
#### 1. 影像域補齊（Image Prop）：
有洞的原始影片在像素層級透過前後影格硬搬像素，得到補齊大半的半成品影像。

#### 2. 編碼器（Encoder）：
半成品影像送入 Encoder，產出單格獨立的特徵 feature_image。

#### 3. 特徵域傳遞（Feature Prop 模組）：
feature_image 進入該模組，透過光流導引的 DCN 做鄰格對齊與融合，產出時序對齊特徵 feature_fusion。
(到這裡為止，完成了論文所說的「雙域傳遞」)

#### 4. 稀疏 Transformer 精修（MSVT Blocks）：
feature_fusion 接著被切成空間視窗，送進 8 個 MSVT 區塊。這一步專門去計算長距離的時空注意力（只算碰到遮罩的區域以節省算力），負責無中生有、修補那些光流搬不過來的複雜背景與細節。

#### 5. 解碼器（Decoder）：
MSVT 精修後的最終特徵，才正式被 Decoder 升採樣解碼回完整的清晰影片。
