...
### scaffold 優化會不會洩漏任務知識，而非改善記憶

scaffold loop 讓 frontier meta-LLM 讀完整軌跡後改寫程式碼、prompt、檔案 schema 與記憶 action surface；它的接受門檻只約束 progression 是否上升，不約束提升是**怎麼**得來的。因此，理論上它可以把任務特有策略提示、偏好的搜尋 query、近似答案的字串或同義詞清單直接硬編進 harness，讓表面上的記憶增益有一部分其實是任務 policy 增益。論文中預先填入 NetHack `strategy` reference（「找到樓梯往下走」）就是任務知識進入 scaffold 的一個良性、明示例子。程序化重生的世界使「死背固定地圖」較不可信，卻不能排除利用反覆出現的任務結構或固定 eval seed。更強的評估應 audit 每個 scaffold diff 是否注入任務知識、禁止攜帶答案的 prompt/schema 新增、只在 development seed 上優化，並報告最終 held-out seed 結果。

### 缺少成熟 retrieval baseline

AutoMem 比較的是自己的 file-system-memory v0 與 sliding-window context baseline，不是成熟的 retrieval stack，例如 chunked episodic memory + BM25、BM25/vector hybrid retrieval、query rewriting/reranking，或 deterministic 的 structured state store。因此，結果能證明的是「這套經優化的 file-system harness」勝過自己的 v0 與 sliding window；它**不能**證明勝過強 RAG/BM25 記憶系統。關鍵 ablation 應固定一套成熟 retrieval harness，再分別測試：(a) AutoMem-style memory specialist，(b) scaffold optimization，並讓所有變體在 held-out seed 上評估。這才能辨別增益究竟來自學到的記憶管控、較好的 retrieval substrate，還是任務特化的 harness engineering。
