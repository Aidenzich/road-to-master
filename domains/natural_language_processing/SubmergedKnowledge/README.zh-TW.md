...
> **校勘註（2026-07-26）**：本筆記先前版本聲稱「Figure 4（LLaMA3-8b, DBPedia）畫出來的其實是 LLaMA3-70b 的數據、與論文表格對不上」。經比對論文**實際的 Figure 4**，此指控為**事實錯誤**：Figure 4 的 Head 為 18.9 / 48.3 / 61.3 / 83.4 / 90.5（k=1,5,10,50,100），與本筆記所引的表格值（48.3 / 83.4 / 90.5）**完全一致**——圖與表本來就吻合，並無 70b 混入。該錯誤主張已移除。此外，本次校勘已把 repo 內**所有隨附圖檔（Figure 2/3/5/6/7）逐一與內文比對，數值均相符**（例如 Figure 3a 的 accuracy 9.8%、Figure 3b 的 Hits@100 名次、Figure 7 的無資訊比例 56/61/65% 皆吻合）；**唯一仍無法在此核對的是論文的正文與未隨附為圖檔的表格**（摘要/introduction 的文字、Section 4 措辭、原註提到的「70b 表格列」等），引用該類敘述時請對照原文 PDF。
...
這張表就是全文論點的縮影：把門檻從 top-5 放到 top-100，同一個模型對同一批題目的「命中率」從 48.3% 跳到 90.5%。作者的解讀是，標準 accuracy 幾乎接近底部，但正確答案其實密集地躺在前一百個 token 裡。這段「top-1 選不出來、但候選集裡有」的落差，就是他們定義的 submerged knowledge。作為對照，LLaMA3-70b 在 DBPedia-Head 的 Hits@100 達 92.1%，而較舊的 LLaMA2-70b 只有 70.5%。

論文 Figure 4 是同一組設定（LLaMA3-8b、DBPedia）的招牌插圖，直接把 $k=1,5,10,50,100$ 的 Hits@k 一字排開；Head 從 Hits@1 的 18.9% 一路升到 Hits@5 的 48.3%、Hits@100 的 90.5%，最陡的一段就發生在 k=1→5 之間。這組圖上的數字與上表同一模型的表格值（Hits@5 48.3、Hits@50 83.4、Hits@100 90.5）完全一致，圖與表彼此吻合。

![Figure 4：論文為 LLaMA3-8b 在 DBPedia 畫的 Hits@k 長條圖，Head/Torso/Tail 隨 k=1,5,10,50,100 遞增；Head 在 k=1 為 18.9%、k=5 躍升至 48.3%、k=100 達 90.5%](imgs/fig4_k_selection.png)
...
### 招牌現象成立，但要質疑的是「量測方式」而非「圖表打架」

「知道卻表達不出來」是一個真實且被低估的現象，把它獨立於 accuracy 來量測也有價值——這點先確立。（更正：本筆記先前版本曾在此聲稱「Figure 4 其實畫的是 70b 數據、與表格對不上」，此為事實錯誤並已移除；經回論文核對，Figure 4 的 Head 為 18.9 / 48.3 / 61.3 / 83.4 / 90.5，與表格的 48.3 / 83.4 / 90.5 完全一致，圖與表本無矛盾。）因此該挑戰的不是子虛烏有的「圖表不一致」，而是這個現象的**絕對量級有多可信**——而那取決於下面兩件事：Hits@k 的命中判準是否過鬆、以及它是否只是需要標準答案的 oracle 診斷。這兩點才是真正削弱結論可用性的地方，詳見後述。
...
文中仍有幾處不一致值得警惕，且多數可用隨附圖檔證實。其一，若正文宣稱「當 $k=50$，head、torso、tail 的 Hits@k 皆超過 80%」，則與 Figure 4 所示 LLaMA3-8b 在 DBPedia 的 Torso（79.6）、Tail（76.6）不符——這兩者其實低於 80%。其二，論文若把 standard accuracy 等同於 Hits@1，也對不上自己的圖：Figure 3(a) 給 LLaMA3-8b／DBPedia-Head 的 accuracy 是 9.8%，Figure 4 給同一模型的 Hits@1（Head）卻是 18.9%，兩者並非同一個量。其三，實驗段若說受測模型「參數規模從 1.5B 到 70B」，Figure 3 的模型清單卻含 72B 的 Qwen2-72b（accuracy 17.3%），連上界都對不齊。另有兩處純文字瑕疵（本次無法從圖檔核對，僅保留原註）：第 4 節把資料集寫成「DBLP」而非 DBPedia；LaTeX 仍用 `iclr2025_conference` 模板卻發表於 ICLR 2026。這些都不是致命錯誤，但引用絕對數值時仍值得保留戒心。
...
- 最需要保留的保留意見：命中判準是「三個連續字元的子串比對 + top-100」（詞表約 12.8 萬 token），寬鬆到可能把字串巧合當成知識；而且 Hits@k 需要先知道答案才算得出來，是 oracle 診斷指標，不等於可部署的改進；回收增益又小又不穩（LLaMA3-8b 僅 +3.8、Mistral 幾乎為 0）。
