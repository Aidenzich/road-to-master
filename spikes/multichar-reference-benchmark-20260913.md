...
- 單人組有實驗設計缺陷：作者把多角色模板的複數措辭帶進單人任務，造成相互衝突的人數指示。這批單人變多人的結果應先歸為提示詞設計問題，不作為 Qwen／H3 單人生成能力較差的證據；也不能以 Codex 較能容忍這份矛盾提示詞推導一般能力排名。原始成果只作診斷紀錄，修正前不納入模型能力結論。
...
**單人組排除聲明：下表仍保留原始觀察分數以供追溯，但人數＝1 的各模型資料均屬提示詞設計有缺陷的診斷組，不是有效的模型能力比較。按動作或重複批次彙總而包含這些單人案例的數據，同樣不能拿來作能力排名。** 單人數值從開頭結論表移除；不改寫原始請求或把事後改好的 prompt 冒充當時輸入。
...
本表是歷史評分重現情況，仍包含提示詞有缺陷的單人組；重複出現同一錯誤不會消除提示詞混雜，不能視為模型缺陷的重複驗證。
